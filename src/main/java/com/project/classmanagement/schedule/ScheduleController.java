...
public class ScheduleController {
	...
	public String schedule(Model model, HttpServletRequest request) throws Exception {
		...
		Iterator<Integer> iteratorKey = result.keySet().iterator(); // 키값 오름차순 정렬(기본)
		...
		while (iteratorKey.hasNext()) {
			...
			if (key > mTime) {
				if (result.get(key).getSchedule_Type().equals("시험")) {
					ExamEvent.add(result.get(key));
				} else if (result.get(key).getSchedule_Type().equals("과제")) {
					TaskEvent.add(result.get(key));
				} else {
					OffDayEvent.add(result.get(key));
				}
			}
		}
